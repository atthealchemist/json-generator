...
export default class JsonGenerator {
    ...
    for (let key in json) {
      ...
      switch (this.parser.getObjectType(key)) {
        ...
        case ObjectType.REPEAT:
          ...
          console.log(
            "ProcessJson.ObjectType::REPEAT.repeatCount",
            repeatCount
          );
          ...
          console.log(
            "ProcessJson.ObjectType::REPEAT.json[arrayKey]",
            json[arrayKey]
          );
      ...
      switch (command.type) {
        ...
        case ObjectType.COMMAND:
          ...
          if (command.manyCommands) {
            console.log(
              "ProcessJson.ObjectType::COMMAND.manyCommands",
              command
            );
          }
          ...
          if (command.hasArgs) {
            ...
            if (matches && matches.length > 0) {
              json[key] = command.origin.replace(
                REGEX_MATCH_COMMAND,
          ...
          if (matches && matches.length > 0) {
            json[key] = json[key].replace(
              ...
              (pointer: string) => {
                ...
                console.log(
                  "ProcessJson.ObjectType::POINTER.replace.pointerKey",
                  pointerKey
                );
            ...
            console.log(
              "ProcessJson.ObjectType::POINTER.value => NOT_MATCH",
              json[key]
            );
          }
          break;
        ...
        case ObjectType.PARENT:
          ...
          json[key] = json[key].replace(REGEX_MATCH_PARENT, (mom: string) => {
            ...
            console.log(
              "ProcessJson.ObjectType::PARENT.parentVal",
              parent[parentKey]
            );
  ...
  public generateJson(document: string): string {
    let json: string = JSON.parse(document);
